...
exports.fetchFullBookDetails = exports.fetchBookDetails = exports.fetchLibrary = void 0;
...
async function fetchBooksPage(client, url) {
    ...
    return {
        books: body.itemsList.map((book) => new book_js_1.KindleBook(book)),
        sessionId,
        paginationToken: body.paginationToken,
    };
}
/** Fetches books from the Kindle library with pagination support. */
async function fetchLibrary(client, args) {
    const query = {
...
async function fetchBookDetails(client, book, options) {
    const version = options?.clientVersion ?? DEFAULT_CLIENT_VERSION;
    const url = (0, urls_js_1.buildStartReadingUrl)(book.asin, version);
    ...
    return {
        ...
        largeCoverUrl: (0, book_js_1.toLargeImageUrl)(book.productUrl),
...
async function fetchFullBookDetails(client, book, partialDetails) {
...
exports.fetchFullBookDetails = fetchFullBookDetails;

...
function buildLibraryUrl(query: Query, filter: Filter): string {
...
export async function fetchFullBookDetails(
  client: HttpClient,
  book: KindleBook,
  ...
  const meta =
    client.parseJsonpResponse<KindleBookMetadataResponse>(response);
